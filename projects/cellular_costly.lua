--[==[
	CA where changing state costs something

	TODO:
		hash based customization bias for states (e.g. [0,1,15,78,...])
...
function update_rules()
	if use_actual_rulebook then
		local rule_def={}
		for i,v in pairs(ruleset) do
			table.insert(rule_def,gen_rule_def(i,v))
		end
		--local actual_max_rule=#ruleset[1]
		generated_rule_definition=table.concat(rule_def,"\n")
		local rule_logic={logic_preamble(),string.format("state_id=clamp(state_id,0,%d);if(false);",max_rule)}
		for i,v in pairs(ruleset) do
			table.insert(rule_logic,gen_rule_logic(i,v))
		end
		...
		--local rule_logic={logic_preamble(),string.format("state_id=clamp(state_id,0,%d);",max_rule),"new_cell=hash_based_rule(state_id,state_hash_offset[my_cell_i],my_cell_i);\n"}
	...
	--local rule_logic={logic_preamble(),string.format("state_id=clamp(state_id,0,%d);if(false);",actual_max_rule)}
...
function random_rule_state(cur_state)
	local ret={}
	local chance_stay=0.1
	local chance_random=0.4
	...
	for i=0,max_rule do
		...
		if r>1-chance_random then
			cur_rule=math.random(1,max_state)
		elseif r>1-(chance_advance+chance_random) then
			cur_rule=(cur_state+1)% max_state
		...
		--]==]
		-- [==[
		if cur_state==0 and i==0 then
			cur_rule=0
		end
		--]==]
		-- [==[ mark invalid states
		local state_decomp=decomp_rule_id(i)
		local sum=0
		for i,v in ipairs(state_decomp) do
			sum=sum+v
		end
		if sum>checked_cells then
			cur_rule=-1
		end
		--]==]
		--if cur_rule>=0 then
			table.insert(ret,cur_rule)
		--end
	end
	return ret
end
