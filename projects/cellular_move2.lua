--[===[
    CA that have these "commands":
        * FW - move forward 1 cell/tick
        * Stop - stop
        * CCW rotate movement direction 1/8 rotation in counter clockwise dir
        * CW same but clockwise dir
        * Delayed CCW/CW a n tick delayed command
    Basically has this possible data:
        * Stopped = 0
        * Moving in direction = 1-8
        * cw/ccw is instant and thus has no data
        * cw/ccw delayed is bits 4+ (i.e last bit is direction earlier bits how many ticks to apply)

    Rules can be:
        * simple: type A is near type B then A set data to some value
        * I  : type A and data X, near type B data Y
        * II : type A near type B and angle 1-8
        * III: as above but near or far (i.e. distance in 8 directions)
        * IV : as above but allow change A->C
        * V  : as above but only allow change on collision (i.e. attempt to enter same cell)
        * VI : increase/decrease delay if it exists

    ISSUES:
        if particle is STOPPED when the rules have no "correct" direction to apply!
            * forbid stopped (meh...)
            * random direction (non-symetric?)
            * new commands: push-pull, sets dir to forward/backwards to the target

    V1:
        Rules are: 
            for each particle type (source)
                for each distance
                    optionally other type (target):
                        do one of:
                            set rotation to forward/backward to the target
                            stop
                            ccw/cw
                            ccw/cw with delay of X
        Possible things:
            electron gets captured from >5
            rotates around proton between 4-3
            electron is pushed from <3
        ISSUES: what if more than one rule applies:
            * use first
            * use last
            etc...

    V1a: same but rule also depends on the angle (1-8) when getting target
    V2: same as V1a but each setup is defined as in cellular_move.lua (i.e. 8 ints and distance)    
--]===]
require 'common'
require 'bit'
...
function update_buffers()
    if particles_pos==nil or particles_pos.w~=max_particle_count then
        ...
        particles_data=make_rgba_buffer(max_particle_count,1)
        --format is: type,subtype,command/direction,age
        is_remade=true
        need_clear=true
    end
...
layout(location = 1) in vec4 particle_data;
...
--[==[ commands are encoded as such:
    0b<XXXX><YYY><Z>
    where:
    * X is bits of current move direction (0-8)
    * Y is bits for the delay (0-7)
    * Z is a bit for rotation direction (i.e. cw or ccw)
--]==]
max_delay=7
function encode_command(direction,delayed_rotation,delay)

    delay=delay or 0
    delayed_rotation=delayed_rotation or 0
    if delay>max_delay then
        delay=max_delay
    end
    if delayed_rotation>1 then delayed_rotation=1 end
    return direction+delayed_rotation*128+delay*16
end
function decode_command(cmd)
    return bit.band(cmd,15),bit.lshift(bit.band(cmd,128),7),bit.band(bit.lshift(cmd,4),max_delay)
end
--calculate rule such that dir=1 is forward for the particle
...
--get first 4 bytes from cmd
function extract_direction( cmd )
    local dir,r,del=decode_command(cmd)
    return dir
end
--update command: lower delayed cmd delay and if ==1 then apply cw/ccw rotation
function update_command(cmd)
    local dir,r,del=decode_command(cmd)
    if del>=1 then
        --lower the delay
        return encode_command(dir,r,del-1)
    elseif del==1 then
        --apply the rotation
        return encode_command(rotate_dir(dir,r*2-1),0,0)
    else
        --no delayed rotation needed
        return encode_command(dir,0,0)
    end
end
function particle_step(  )
    ...
    --recalc rules/commands
    local trg_pos={} --lookup cache of "stuff"
    for i=0,current_particle_count-1 do
        local data=particles_data:get(i,0)
        local current_command=data.g
        local pos=fix_pos(particles_pos:get(i,0))
        --wrong here::: rule output and command is not the same. Rule changes current direction!
        ERROR(HERE!!!)
        local command=calculate_rule(pos) or current_command
        command=update_command(command)
        local dir=extract_direction(command)

        local tpos=displace_by_dir(pos,dir)
        local sl=static_layer:get(tpos.r,tpos.g)
        if sl.a>0 then
            dir={0,0}
            tpos=displace_by_dir(pos)
        end
        trg_pos[i]={command,tpos}
        --increment particles attempting to enter tile
    ...
    --actually update the particles
    for i=0,current_particle_count-1 do
        local pos=fix_pos(particles_pos:get(i,0))
        local tpos=trg_pos[i][2]
        local command=trg_pos[i][1]
        local dir=extract_direction(command)
        local tp=movement_layer_target:get(tpos.r,tpos.g)
...
function scratch_update(  )
    ...
    place_pixels_shader:push_attribute(particles_data.d,"particle_data",4,GL_BYTE)
...
function place_atom_wlayers( target_x,target_y,size,seed )
    ...
    while bs>0 do
        ...
        for i=1,math.min(#l,bs) do
            ...
            particles_data:set(current_particle_count,0,{atom_type,variation,0,1})
